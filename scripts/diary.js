 ...
 * @LastEditors: 曾茹菁
 * @LastEditTime: 2022-08-30 10:33:50
 */
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";
import { cwd } from "node:process";
const DIARY_LINK = "/daily/diary";
const DIARY_PATH = "docs/daily/diary"; // 日记路径
const JSON_PATH = "docs/daily/diary/index.json"; // 边栏文件路径
const cedPath = cwd();
const diaryPath = resolve(cedPath, DIARY_PATH);
/**
 * @description: 获取今天日期
...
function init() {
  const jsonPath = resolve(cedPath, JSON_PATH);
  ...
  let jsonContent = JSON.parse(readFileSync(jsonPath, "utf-8"));
